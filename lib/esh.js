...
function subdomainRegex(baseUrl){
	return regex = new RegExp("((?!www)\\b[\\w\\.]+)\\." + eshUtils.escapeRegExp(baseUrl) + "(?::)?(?:\d+)?");
}
...
module.exports = function(options){
	...
	if(options.logger) console.log("express-subdomain-handler: Rewriting all subdomain routes to /%s/[subdomain1]/[subdomain2]/path/after/baseurl for baseUrl %s", options.prefix, options.baseUrl);
	...
	return function(req, res, next){
		...
		if(options.logger) console.log('Full url: %s, Express route: %s', originalUrl, req.url);
